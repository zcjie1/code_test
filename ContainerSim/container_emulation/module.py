...
class Mac80211Hwsim(object):
    ...
    def create_hwsim(self, n):
        self.get_phys()
        p = Popen(["/home/gongtao/Desktop/container_network2/mac80211_hwsim_mgmt/hwsim_mgmt/hwsim_mgmt", "-c", "-n", self.prefix + ("%02d" % n)],
    ...
    def docker_config(self, nradios=0, nodes=None, dir='~/',
        ...
        sh("echo 'do' >> %s" % file)
        sh(r"echo '    pid=$(ps -aux | grep \"${nodes[$j]}\" | grep -v 'hostapd' "
           r"| awk \"{print \$2}\" | awk \"NR>=%s&&NR<=%s\")' "
           ">> %s" % (num+1, num+1, file))
            ...
            for wlan in range(len(node.params['wlan'])):
                ...
                if False:
                    ...
                    if 'docker' not in params:
                        ...
                        sh('iw phy {} set netns name {}'.format(phys[id], node.name))
                    node.cmd('ip link set {} up'.format(wlan_list[0]), stdout=True, stderr=True, stdin=False, tty=True)
                    node.cmd('ip link set {} name {}'.format(wlan_list[0], node.params['wlan'][wlan]), stdout=True, stderr=True, stdin=False, tty=True)
                wlan_list.pop(0)
                phys.pop(0)
        except:
            exception("Warning:")

...
static int dpdk_eal_init(struct config *cfg, char *argv0)
{
    ...
    if (cfg->ports[0].is_vdev) {
        // printf("The %s vdev is used.\n", cfg->ports[0].pci);
        sprintf(vdev, "--vdev=%s,path=/var/run/usvhost0,queue_size=1024", cfg->ports[0].pci);
        // printf("The %s vdev is used.\n", vdev);
        argv[argc++] = vdev;
        argv[argc++] = nopci;
    }

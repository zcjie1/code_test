...
#define ZCIO_LOG_PATH "/var/log/zcio.log"
...
static void parse_pkt(struct rte_mbuf *pkt, FILE *log)
{
    static uint64_t pkt_num;
	struct rte_ether_addr src_mac, dst_mac;
    char sip[INET_ADDRSTRLEN];
    char dip[INET_ADDRSTRLEN];
	
	struct rte_ether_hdr *eth_hdr = mbuf_eth_hdr(pkt);
    struct rte_ipv4_hdr *ip_hdr = mbuf_ip_hdr(pkt);
    struct rte_udp_hdr *udp_hdr = mbuf_udp_hdr(pkt);
	src_mac = eth_hdr->src_addr;
	dst_mac = eth_hdr->dst_addr;
    inet_ntop(AF_INET, &ip_hdr->src_addr, sip, INET_ADDRSTRLEN);
	inet_ntop(AF_INET, &ip_hdr->dst_addr, dip, INET_ADDRSTRLEN);
    char *data_end = (char *)(udp_hdr) + ntohs(udp_hdr->dgram_len);
    *data_end = '\0';

    fprintf(log, "--------------------------UDP PACKET %lu--------------------------\n", pkt_num++);
    fprintf(log, " [%02x:%02x:%02x:%02x:%02x:%02x||%s||%u] -- > [%02x:%02x:%02x:%02x:%02x:%02x||%s||%u]\n", 
            RTE_ETHER_ADDR_BYTES(&src_mac), sip, ntohs(udp_hdr->src_port), 
            RTE_ETHER_ADDR_BYTES(&dst_mac), dip, ntohs(udp_hdr->dst_port));
    fprintf(log, "DATA: %s\n", (char *)(udp_hdr + 1));
    fprintf(log, "-----------------------------------------------------------------\n\n");
}
...
static int message_log(void *arg __rte_unused)
{
    ...
    FILE *log = fopen(ZCIO_LOG_PATH, "w+");
    if(log == NULL) {
...
int main(int argc, char *argv[])
{
	...
	rte_eal_remote_launch(message_log, NULL, worker_id);

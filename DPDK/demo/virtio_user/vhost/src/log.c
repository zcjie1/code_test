...
static void show_results(void *param)
{
...
static void period_show_stats(void *param)
{
...
// 生成日志目录
static int generate_logdir(char *dir_path) {
    struct stat info;
...
void log_init()
{
    if(generate_logdir(LOG_DIR) != 0)
		rte_exit(EXIT_FAILURE, "Fail to create Log Directory.\n");
    
    FILE *logfile = fopen(LOG_FILE, "w");
    statistics.log = logfile;

    rte_eal_alarm_callback cb = period_show_stats;
    rte_eal_alarm_set(PERIOD * US_PER_S, cb, &statistics);
}
...
void log_exit()
{
    ...
    // 注销定期报告函数
    rte_eal_alarm_cancel(period_show_stats, &statistics);

    // 关闭日志文件
    fflush(statistics.log);
    fclose(statistics.log);
}